...
class Strategy:
    ...
    def search(self):
        ...
        # goal = get_goal()
        # if goal:
        #     angle = (Vector2(goal.cx(), goal.cy()) - center).get_angle_degrees()

        # uart.write(f"V,0,0.4,{angle}\n")
        # return
        if ball:
            self.frames_no_ball = 0
            ...
            if diff.mag < 20 and abs(ball_angle) < 15:
                self.state = "attack"
                return
            elif diff.mag < 5:
                self.state = "attack"
                uart.write(f"V,0,0.3,{ball_angle}\n")
                return

        diff += self.stay_in_bounds() * 100
